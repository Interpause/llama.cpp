...
std::vector<std::unique_ptr<field>> make_llama_cmpl_schema(const common_params & params_base, task_params & params) {
    ...
    add((new field_str("pre_trigger_grammar"))
        ->set_desc("See server-common.cpp for this being set from body[grammar]")
        ->set_handler([&](field_eval_context & ctx, const json & data) {
            ctx.params.sampling.pre_trigger_grammar = json_value(data, "pre_trigger_grammar", std::string());
        }));
